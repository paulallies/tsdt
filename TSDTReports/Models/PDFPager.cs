...
class MyPageEvents : PdfPageEventHelper
{
    ...
    {
        String text = "";
        if (document.PageSize.Width > document.PageSize.Height) //if the report is landscape. timesheet report is the only landscape report
        {
            text = "Signature:";
            cb.BeginText();
            cb.SetFontAndSize(bf, 8);
            cb.SetTextMatrix(25, 60);
            cb.ShowText(text);
            cb.EndText();
            ...
            cb.BeginText();
            cb.SetFontAndSize(bf, 8);
            cb.SetTextMatrix(300, 60);
            cb.ShowText(text);
            cb.EndText();

            text = "Chevron Approval:";
            cb.BeginText();
            cb.SetFontAndSize(bf, 8);
            cb.SetTextMatrix(25, 30);
            cb.ShowText(text);
            cb.EndText();
        ...
        else
        {
            ...
            cb.BeginText();
            cb.SetFontAndSize(bf, 8);
            cb.SetTextMatrix(300, 25);
            cb.ShowText(text);
            cb.EndText();
            cb.AddTemplate(template, 300 + len, 25);
            cb.BeginText();
            cb.SetFontAndSize(bf, 8);
            cb.SetTextMatrix(300, 820);
            //if (pageN % 2 == 1)
            //{
            //    cb.ShowText("Report");
            //}
